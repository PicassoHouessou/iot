...
export const useFiltersQuery = () => {
    ...
    useEffect(() => {
        setQuery((prevState: any) => ({...prevState, page: pagination.current, itemsPerPage: pagination.itemsPerPage}));
    }, [pagination]);
    ...
    useEffect(() => {
        if (query) {
            ...
            const queryItemsPerPage = urlSearchParams.get("itemsPerPage") ?? query.itemsPerPage ?? initialPagination.itemsPerPage;
            if (
                (queryLength === 0 ||
                    (queryLength === 1 && queryItemsPerPage == initialPagination.itemsPerPage) ||
                    (queryLength === 2 &&
    ...
    return {
        sortData, resetSortData, resetAllQuery,
        pagination, setPagination, handleSearch, setSearchFormValue, searchFormValue,
        query, setQuery, searchParams, setSearchParams, canReset, setCanReset
    }
}
...
export const useHandleTableChange = ({
    ...
    const navigate = useNavigate();
    return useCallback((pagination: TablePaginationConfig, filters: any, sorter: any) => {
        sortData(sorter?.field, sorter?.order);

        if (pagination.pageSize) {
            setPagination((prevState: any) => ({...prevState, itemsPerPage: pagination.pageSize!}));
        }
        if (pagination.current) {
            ...
            setPagination((prevState: any) => ({...prevState, current: pagination.current!}));
    ...
    }, [path, sortData, navigate, setTableParams, setData, setPagination, tableParams.pagination?.pageSize]);
