    ...
    return (
        <div className="page-auth">
            <div className="header">
                <Container>
                    <Link to={AdminPages.DASHBOARD} className="header-logo">{APP_NAME}</Link>
                ...
                <Container>
                    <Card className="card-auth">
                        <Card.Body className="text-center">
                            ...
                            {message && message.error && <Alert variant="danger">{message.error}</Alert>}
                            {message && message.success && <Alert variant="success">{message.success}</Alert>}
                            <Card.Title>{t("Réinitialiser votre mot de passe")}</Card.Title>
                            <Card.Text
                                className="mb-5">{t("Saisissez votre adresse e-mail et nous vous enverrons un lien pour réinitialiser votre mot de passe.")}</Card.Text>

                            <Form onSubmit={handleSubmit}> <Row className="g-2">
                                <Col sm="8">
                                    <Form.Control type="text"
                                                  placeholder={t("Saisir votre adresse e-mail")} value={formValue}
                                                  onChange={(e) => setFormValue(e.target.value)}/>
                                </Col>
                                <Col sm="4"><Button variant="primary" type="submit">{t("Réinitialiser")}</Button></Col>
                            </Row>
                            </Form>
