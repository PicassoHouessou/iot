...
export default function Signin() {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (
                res?.user &&
                (res.user?.roles?.includes("ROLE_ADMIN") || res.user?.roles?.includes("ROLE_USER"))
            ) {
                dispatch(setCredentials({user: res.user}));
                dispatch(setTokenCredentials({token: res?.token, refresh_token: res?.refresh_token}));
                navigate("/dashboard")
            } else {
        ...
        <div className="page-sign">
            <Card className="card-sign">
                <Card.Header>
                    ...
                    <Card.Text>Welcome back! Please signin to continue. You can use
                        <ul>
                    ...
                    <Form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            ...
                            <Form.Control type="text" placeholder="Enter your email address"
                                          value={email} onChange={setFormChange}/>
                        </div>
                        <div className="mb-4">
                            <Form.Label className="d-flex justify-content-between">Mot de passe <Link to="">Mot de passe
                                oublié?</Link></Form.Label>
