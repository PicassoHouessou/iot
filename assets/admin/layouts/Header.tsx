...
export default function Header({onSkin}: { onSkin: React.Dispatch<React.SetStateAction<string>> }) {
    ...
    return (
        <div className="header-main px-3 px-lg-4">
            <Link to="" onClick={toggleSidebar} className="menu-link me-3 me-lg-4"><i
                className="ri-menu-2-fill"></i></Link>
            ...
            <Dropdown className="dropdown-skin" align="end">
                ...
                <Dropdown.Menu className="mt-10-f">
                    ...
                    <nav className="nav nav-skin">
                        <Link to="" onClick={skinMode}
                              className={localStorage.getItem("skin-mode") ? "nav-link" : "nav-link active"}>Light</Link>
                        <Link to="" onClick={skinMode}
                              className={localStorage.getItem("skin-mode") ? "nav-link active" : "nav-link"}>Dark</Link>
                    </nav>
                    <hr/>
                    <label>Sidebar Skin</label>
                    <nav id="sidebarSkin" className="nav nav-skin">
                        <Link to="" onClick={sidebarSkin}
                              className={!(localStorage.getItem("sidebar-skin")) ? "nav-link active" : "nav-link"}>Default</Link>
                        <Link to="" onClick={sidebarSkin}
                              className={(localStorage.getItem("sidebar-skin") === "prime") ? "nav-link active" : "nav-link"}>Prime</Link>
                        <Link to="" onClick={sidebarSkin}
                              className={(localStorage.getItem("sidebar-skin") === "dark") ? "nav-link active" : "nav-link"}>Dark</Link>
                    </nav>
                </Dropdown.Menu>
            </Dropdown>
